...
import os2 from "os";
import tty from "tty";
import { readdirSync as readdirSync5, existsSync as existsSync5, copyFileSync as copyFileSync2, unlinkSync as unlinkSync2, statSync as statSync2, rmSync as rmSync2 } from "fs";
import { join as joinPath, resolve as resolvePath, basename as getBasename } from "path";
...
var require_sbcs = __commonJS({
  "../node_modules/.pnpm/chardet@2.2.0/node_modules/chardet/lib/encoding/sbcs.js"(exports) {
    ...
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.KOI8_R = exports.windows_874 = exports.windows_1258 = exports.windows_1257 = exports.windows_1256 = exports.windows_1251 = exports.ISO_8859_9 = exports.ISO_8859_8 = exports.ISO_8859_7 = exports.ISO_8859_6 = exports.ISO_8859_5 = exports.ISO_8859_2 = exports.ISO_8859_1 = undefined;
    var match_1 = __importDefault(require_match());
...
var _ = {
  ...
  get: (obj, path4 = "", defaultValue) => {
    const travel = (regexp) => String.prototype.split.call(path4, regexp).filter(Boolean).reduce((res, key) => res == null ? res : res[key], obj);
...
function isPromptConstructor(prompt2) {
  return Boolean(prompt2.prototype && "run" in prompt2.prototype && typeof prompt2.prototype.run === "function");
}
...
function buildScorchedPayload(realSector, junkSectors) {
  ...
  for (const sector of allSectors) {
    ...
    do {
      ...
      attempts++;
      if (attempts > 1e4)
        throw new Error("Cannot place sector: payload too small");
...
function overlaps(pos, len, positions, sectors, _used) {
  for (let i = 0;i < positions.length; i++) {
    ...
    if (pos < otherEnd && thisEnd > otherStart)
      return true;
  }
  return false;
}
...
function encodeSlot(slot) {
  if (slot.tokenSalt.length !== SLOT_SALT_LEN)
    throw new Error("bad salt len");
  if (slot.verifier.length !== SLOT_VERIFIER_LEN)
    throw new Error("bad verifier len");
  if (slot.slotIV.length !== SLOT_IV_LEN)
    throw new Error("bad IV len");
  if (slot.slotTag.length !== SLOT_TAG_LEN)
    throw new Error("bad tag len");
  if (slot.slotCiphertext.length !== SLOT_CT_LEN)
    throw new Error("bad ct len");
...
function hideFiles(opts) {
  ...
  if (tokens.length === 0)
    throw new Error("At least one token required.");
  if (tokens.length > MAX_SLOTS)
    throw new Error(`Maximum ${MAX_SLOTS} tokens allowed.`);
  ...
  const junkSectors = Array.from({ length: decoyCount }, () => buildJunkSector(realSector.length));
...
function printStep(step, detail) {
  console.log(source_default.gray("  \u203A"), source_default.bold.white(step), detail ? source_default.gray(detail) : "");
}
...
function printTable(rows) {
  ...
  for (const [key, val] of rows) {
    console.log("  " + source_default.gray(key.padEnd(maxKey + 2)), source_default.white(val));
  }
}
...
function printExportResult(path4, tokenCount) {
  printSuccess(`Exported ${tokenCount} token(s) to: ${source_default.bold(path4)}`);
...
function spinner(msg) {
  ...
  const interval = setInterval(() => {
    process.stdout.write(`\r  ${source_default.cyan(frames[i++ % frames.length])} ${source_default.gray(msg)}`);
  }, 80);
  return {
    stop: (done) => {
      clearInterval(interval);
      process.stdout.write(`\r  ${source_default.green("\u2714")} ${source_default.white(done ?? msg)}
`);
    }
  };
}
...
var pluginManager = new PluginManager(process.cwd());
async function runHideCommand(files, opts) {
  ...
  if (realPassword) {
    ...
    const answers = await dist_default14.prompt([
      ...
      },
      {
        ...
        mask: "*",
        validate: (val, answers2) => val === answers2["realPassword"] || "Tokens do not match."
      }
    ]);
  ...
  printTokenSummary(tokens.filter((t) => t.type === "real").length, tokens.filter((t) => t.type === "decoy").length);
  console.log();
...
async function runExportTokensCommand(opts) {
  ...
  printWarn("This command creates a plaintext token export. Store it separately and securely.");
  console.log();
  const tokens = [];
  let addMore = true;
  while (addMore) {
...
function runBenchmarkCommand() {
  ...
  const sampleData = randomBytes2(10485760);
...
(async () => {
  ...
  program2.name("hidebaf").description(`HIDEBAF \u2014 Highly Incubated Dragonian Encryptory Binary using an Archive of Files
Scorched Earth encryption with multi-token redirection and AES-256-GCM payloads.`).version("1.0.0");
  program2.command("hide <files...>").description("Encrypt one or more files into a .HIDEBAF archive").option("-o, --output <file>", "Output archive path (default: <first_file>.HIDEBAF)").option("--token <token>", "Token (password) \u2014 optional for non-interactive archiving").option("--hint <text>", "Optional hint stored in archive header (plaintext metadata)").option("--no-compress", "Disable zlib compression of the payload bundle").option("--decoy <password>", "Add a decoy token (repeatable: --decoy pw1 --decoy pw2)", (val, acc) => {
    acc.push(val);
    return acc;
  }, []).option("--export-tokens <path>", "Export all tokens to a .EBFATs file after archiving").action(async (files, opts) => {
    await runHideCommand(files, opts);
  });
  program2.command("reveal <archive>").description("Decrypt a .HIDEBAF archive using a token").option("-o, --output <dir>", "Output directory for extracted files (default: ./hidebaf-output)").option("--token <token>", "Token (password) \u2014 omit to be prompted interactively").action(async (archive, opts) => {
    await runRevealCommand(archive, opts);
  });
  program2.command("export-tokens").description("Interactively build and export a .EBFATs token file").option("-o, --output <path>", "Output path for .EBFATs file (default: tokens-export.EBFATs)").action(async (opts) => {
    await runExportTokensCommand(opts);
  });
  program2.command("import-tokens <ebfats>").description("List tokens stored in a .EBFATs file (passwords not shown)").action(async (ebfatsPath) => {
    await runImportEbfatsCommand(ebfatsPath);
  });
  program2.command("info <archive>").description("Display archive metadata without decrypting (slot count, flags, hint)").action((archive) => {
    runInfoCommand(archive);
  });
  const pluginCmd = program2.command("plugin").description("Manage HIDEBAF base and custom plugins");
  pluginCmd.command("list").description("List all loaded base and custom plugins").action(() => {
    runPluginListCommand();
  });
  pluginCmd.command("add <path>").description("Install a custom plugin into Lib/Custom-Plugins").action(async (p) => {
    await runPluginAddCommand(p);
  });
  pluginCmd.command("remove <name>").description("Remove a custom plugin from Lib/Custom-Plugins").action((n) => {
    runPluginRemoveCommand(n);
  });
  pluginCmd.command("info <name>").description("Show detailed metadata for a plugin").action((n) => {
    runPluginInfoCommand(n);
  });
  program2.command("verify <archive>").description("Cryptographically verify archive structure and checksums").action((a) => {
    runVerifyCommand(a);
  });
  program2.command("benchmark").description("Benchmark local AES-256-GCM encryption & scrypt KDF throughput").action(() => {
    runBenchmarkCommand();
  });
  program2.command("wipe <files...>").description("Securely erase files using multi-pass random data (Scorched Earth shredder)").action((files) => {
    runWipeCommand(files);
  });
  program2.command("audit").description("Display recent operation audit logs from the logger plugin").action(() => {
    runAuditCommand();
  });
  program2.command("compress-stats <archive>").description("Analyze compression ratio and space savings of an archive").action((a) => {
    runCompressStatsCommand(a);
  });
  program2.command("theme [themeName]").description("View or set CLI visual theme (dragon, stealth, cyber, classic)").action((t) => {
    runThemeCommand(t);
  });
